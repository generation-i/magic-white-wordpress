...
<body oncontextmenu="return false; data-scroll-container">
        ...
        <section
            ...
            <?php if ($works->max_num_pages > 1): ?>
                <div class="text-center mt-6">
                    <button class="load-more bg-blue-500 text-white px-4 py-2 rounded"
                        data-term="<?php echo esc_attr($term->slug); ?>"
                        data-page="1">
                        Load More
                    </button>
            ...
            function initVideoHover() {
                document.querySelectorAll('video').forEach(video => {
                    ...
                    video.addEventListener('mouseleave', () => {
                        ...
                        // Force poster to reappear
                        const poster = video.getAttribute('poster');
                        if (poster) {
                            // This trick reloads poster without reloading the video file
                            video.load();
                        }
                    });
                });
                    ...
                    fetch('<?php echo admin_url("admin-ajax.php"); ?>?action=load_more_work&term=' + term + '&page=' + page)
                        ...
                        .then(data => {
                            if (data.trim()) {
                                ...
                                initVideoHover(); // re-init hover play for new videos
                            } else {
                                button.remove();
                            }
                        });
    ...
    <div id="work-details-popup"
        ...
        <div
            class="relative bg-white rounded-lg sm:w-[50%] w-auto max-h-[90vh] px-2 overflow-y-auto shadow-xl mt-16 no-scrollbar">
    ...
    <script>
        ...
        function handleOutsideClick(event) {
            ...
            if (window.innerWidth < 640 && !img.contains(event.target)) {
